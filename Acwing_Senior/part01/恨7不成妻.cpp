...
/*
在对状态转移方程进行分析后发现，当前状态需要上一个状态的三个信息来更新，
因此在记录每一个状态的信息时，只记录平方和是无法对下一个状态进行更新的，总共需要记录三个信息
*/
struct F {
    /*
    s0表示当前状态合法方案（每一位均不为7）的个数
    s1表示当前状态所有合法方案之和
    s2表示当前状态所有合法方案的平方和
    */
    int s0, s1, s2;
...
int dp(LL n) {
    ...
    for (int i = nums.size() - 1; i >= 0; i--) {
        ...
        for (int j = 0; j < x; j++) {
            ...
            res = mod(
                res + 
                (last_a % P) * (last_a % P) % P * power9[i + 1] % P * power9[i + 1] % P * v.s0 % P + v.s2 + 2 * last_a % P * power9[i + 1] % P * v.s1, P
            );
        }
